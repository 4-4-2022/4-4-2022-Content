...
public class JaxWsConfiguration {
	...
	@Bean
	public CupcakeServiceImpl cupcakeServiceImpl() {
		return new CupcakeServiceImpl();
	}
	
	@Bean
	public Endpoint endpoint() {
		EndpointImpl endpoint = new EndpointImpl(springBus(), cupcakeServiceImpl());
		/*
		 * We need to expose the endpoing under an address. We have to publish endpoint ourself; it's not immediately
		 * consumable.
		 */
		endpoint.publish("/cupcake-service");
		return endpoint;
	}
